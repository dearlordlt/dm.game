...
{	
	...
	function saveNpc($npc, $userId) {
		if ($npc->id != 0) {
			$npcId = $npc->id;
			mysql_query("UPDATE npc SET label='$npc->label', last_modified=NOW(), last_modified_by=$userId, category_id=$npc->category_id WHERE id=$npcId;");
			mysql_query("DELETE FROM npc_commands WHERE npc_id=$npcId;");			
		} else {
			mysql_query("INSERT INTO npc (label, last_modified, last_modified_by, category_id) VALUES ('$npc->label', NOW(), $userId, $npc->category_id);");
