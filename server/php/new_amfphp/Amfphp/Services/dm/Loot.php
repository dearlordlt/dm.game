...
class Loot {
    ...
    function saveLoot($loot, $userId) {
        $userId = mysql_real_escape_string($userId);
        $loot->label = mysql_real_escape_string($loot->label);

        if ($loot->id == 0) {
            mysql_query("INSERT INTO loot (label, spawn_period, last_looted, last_modified, last_modified_by, category_id) VALUES ('$loot->label', $loot->spawn_period, NOW(), NOW(), $userId, $loot->category_id);");
            ...
            $lootId = $loot->id;
            mysql_query("UPDATE loot SET label='$loot->label', spawn_period=$loot->spawn_period, last_modified=NOW(), last_modified_by=$userId, category_id=$loot->category_id WHERE id=$lootId;");
