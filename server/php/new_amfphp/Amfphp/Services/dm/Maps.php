...
class Maps {
    ...
    function getUserMaps($userId) {
        ...
        $map_rs = mysql_query("SELECT * FROM maps WHERE id IN (SELECT object_id FROM object_permissions WHERE object_type='map' AND moderator_id=".$userId.");");
        $maps = array();
        while ($map = mysql_fetch_object($map_rs))
            $maps[] = $map;
        return $maps;
    ...
    function saveMap($map, $userId) {
        mysql_query("INSERT INTO maps (label, width, height, skybox_id, last_modified_by, last_modified, active, category_id) VALUES ('$map->label', $map->width, $map->height, $map->skybox_id, $userId, NOW(), 1, $map->category_id);");
        ...
        foreach ($map->avatarSpawnPoints as $avatarSpawnPoint) {
            mysql_query("INSERT INTO map_avatar_spawnpoints (map_id, x, y, z, rotationY) VALUES ($mapId, $avatarSpawnPoint->x, $avatarSpawnPoint->y, $avatarSpawnPoint->z, $avatarSpawnPoint->rotationY);");
