...
/**
 * Annotated Authentication for Amfphp. 
 * This plugin can be deactivated if the project doesn't need to protect access to its services.
 * 
 * On a service object, the plugin looks for a annotations (@Roles) in comments. 
 * If the annotations exists, the plugin will look for a role in the session that matches the role.
 ...
 * <code>
 * 
 * public function someServiceMethod($methodName){
...
class Roles extends Annotation {}

class AmfphpAnnotatedAuthentication {
    /**
    ...
    public function filterServiceObject($serviceObject, $serviceName, $methodName) {
		...
		// just for compatibility with standard Authetincation plugin
        if ($methodName == self::METHOD_GET_METHOD_ROLES) {
            throw new Exception('_getMethodRoles method access forbidden');
        }
		
		$reflectionClass = new ReflectionAnnotatedClass(get_class($serviceObject));
		
		$classRolesAnnotations = $reflectionClass->getAllAnnotations(self::ROLES_ANNOTATION);
		
		$reflectionMethod = new ReflectionAnnotatedMethod(get_class($serviceObject), $methodName);
		
		$methodRolesAnnotations = $reflectionMethod->getAllAnnotations(self::ROLES_ANNOTATION);
		
		if ( ( count($methodRolesAnnotations) == 0 ) && ( count($classRolesAnnotations) == 0 ) ) {
			// no roles found - allowed to execute
			return;
		}
        ...
        $userRoles = array_keys( $_SESSION[self::SESSION_FIELD_ROLES] );
		
		foreach ( $methodRolesAnnotations as $methodAnnotation ) {
			
			if ( isset($methodAnnotation->value) ) {
				if (is_string($methodAnnotation->value) ) {
					if ( in_array($methodAnnotation->value, $userRoles) ) {
						return;
					}
				} else if ( is_array($methodAnnotation->value ) ) {
					if (count( array_intersect($userRoles, $methodAnnotation->value ) ) > 0 ) {
						return;
					}
				}
				 // $userRoles[$methodAnnotation->value];
				// return;
		...
		if (count($methodRolesAnnotations) > 0 ) {
			throw new Amfphp_Core_Exception('Access denied.');
		}
		
		foreach ( $classRolesAnnotations as $classAnnotation ) {
			
			if ( isset($classAnnotation->value) ) {
				if (is_string($classAnnotation->value) ) {
					if ( in_array($classAnnotation->value, $userRoles) ) {
						return;
					}
				} else if ( is_array($classAnnotation->value ) ) {
					if (count( array_intersect($userRoles, $classAnnotation->value ) ) > 0 ) {
						return;
					}
				}
