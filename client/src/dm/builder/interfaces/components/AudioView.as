package dm.builder.interfaces.components 
{
	 ...
	 * @author zenia
	 */
	public class AudioView extends BuilderWindow 
	{
		private var _audio:Audio;
		
		public function AudioView(parent:DisplayObjectContainer, audio:Audio) 
		{
			_audio = audio;
			super(parent, "AudioView");
		...
		public override function initialize (  ) : void {
			...
			if (_audio.id == 0) {
				createDummy();
				var select_btn:PushButton = new PushButton(_dummy, 0, 0, "Select Audio", onSelectAudioBtn);
				select_btn.x = _width * 0.5 - select_btn.width * 0.5;
				select_btn.y = bodyHeight * 0.5 - select_btn.height * 0.5;
			} else {
				var msg_lbl:BuilderLabel = new BuilderLabel(_body, 10, 10, "Audio '" + _audio.label + "' was assigned to this entity. If you want to modify this component, use 'AudioEditor' in 'Tools' menu. Also make sure entity has Skin3D component.");
		...
		private function onSelectAudioBtn(e:MouseEvent):void 
		{
			new SelectAudio(parent.parent.parent.parent);
		}
		...
		private function onCloseBtn1(e:MouseEvent):void {
			...
			EntityManager.instance.currentEntity.remove(Audio);
		}
